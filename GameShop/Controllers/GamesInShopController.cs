...
namespace GameShop.Controllers
{
    ...
    public class GamesInShopController(IHttpContextAccessor httpContextAccessor,
        ...
        IGameStatusService gameStatusService) : Controller
    {
        ...
        public async Task<IActionResult> RecommendationGames()
        {
            var recommendedGameProducts = await gameShopContext.RecommendedGameProducts
                ...
                .ToListAsync();
            var listGameProductsByExperts = new Dictionary<NameSurname, List<GameProduct>>();

            foreach (var item in recommendedGameProducts)
            {
                var nameSurname = new NameSurname(item.ExpertName, item.ExpertSurname);
                if (!listGameProductsByExperts.TryGetValue(nameSurname, out List<GameProduct>? value))
                {
                    listGameProductsByExperts.Add(nameSurname, [item.GameProduct]);
                }
                else
                {
                    value.Add(item.GameProduct);
                }
            }
            return View(listGameProductsByExperts);
        }
